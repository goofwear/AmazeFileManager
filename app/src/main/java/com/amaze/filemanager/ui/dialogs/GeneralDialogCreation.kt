...
object GeneralDialogCreation {
    ...
    fun deleteFilesDialog(
        ...
        val dialog = MaterialDialog(c).show {
            ...
            negativeButton(text = c.getString(R.string.cancel).toUpperCase())
            positiveButton(
                text = c.getString(R.string.delete).toUpperCase(),
                click = {
...
//                pieData.setValueTextColor(if (isDarkTheme) Color.WHITE else Color.BLACK)
    ...
    fun showEncryptWarningDialog(
        ...
        MaterialDialog(main.requireContext()).show {
            ...
            getActionButton(WhichButton.POSITIVE).setTextColor(main.mainActivity.accent)
            getActionButton(WhichButton.NEGATIVE).setTextColor(main.mainActivity.accent)
        }
    }
    ...
    fun showCompressDialog(
        ...
        MaterialDialog(m).show {
            ...
            WarnableTextInputValidator(
                ...
                val isValidFilename = FileUtil.isValidFilename(text)
    ...
    @JvmStatic
    fun showSortDialog(m: AppsListFragment, appTheme: AppTheme) {
        val accentColor = (m.activity as ThemedActivity?)!!.accent
        val sort = m.resources.getStringArray(R.array.sortbyApps)
        val current = m.Sp.getString("sortbyApps", "0")!!.toInt()
        MaterialDialog(m.requireActivity()).show {
            title(R.string.sort_by)
            listItemsSingleChoice(
                items = sort.toList(),
                initialSelection = if (current > 2) current - 3 else current,
                selection = { dialog, which, text -> }
            )
            positiveButton(
                res = R.string.descending,
                click = {
//                m.Sp.edit().putString("sortbyApps", "" + (getSelectedIndex() + 3)).apply()
                    m.getSortModes()
                    m.loaderManager.restartLoader(AppsListFragment.ID_LOADER_APP_LIST, null, m)
                    dismiss()
                }
            )
            negativeButton(
                res = R.string.ascending,
                click = {
//                m.Sp.edit().putString("sortbyApps", "" + getSelectedIndex()).apply()
                    m.getSortModes()
                    m.loaderManager.restartLoader(AppsListFragment.ID_LOADER_APP_LIST, null, m)
                    dismiss()
                }
    ...
    fun showHistoryDialog(
        ...
        MaterialDialog(m.requireActivity()).show {
            ...
            getActionButton(WhichButton.POSITIVE).setTextColor(m.mainActivity.accent)
            getActionButton(WhichButton.NEGATIVE).setTextColor(m.mainActivity.accent)
        }
    }
